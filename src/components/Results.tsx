...
export function usePollManager({ 
    ...
    try {
      ...
      if (activationError) {
        ...
        if (errorCountRef.current >= 3) {
          const newInterval = Math.min(30000, pollingInterval * 2);
        ...
        if (voteError) {
          ...
          if (currentVoteCount === lastVoteCountRef.current) {
            ...
            if (noChangeCountRef.current >= 3 && pollState === 'voting') {
              const newInterval = Math.min(10000, Math.round(pollingInterval * 1.5));
              if (newInterval !== pollingInterval) {
                if (debugMode) {
  ...
  useEffect(() => {
    ...
    // Set up subscription for real-time updates as a fallback
    try {
